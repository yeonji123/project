...
const Join = ({ navigation }) => {
    ...
    const [errorMessageEmail, setErrorMessageEmail] = useState(""); // email
    ...
    const validateEmail = id => {
      ...
      const handleEmailChange = (email) => {
        const changedEmail = removespace(email);
        setEmail(changedEmail);
        setErrorMessageEmail(
          validateEmail(changedEmail) ? "올바른 아이디 형식입니다." : "이메일 형식을 확인해주세요."
        );
        setOkMail(false);
      }
